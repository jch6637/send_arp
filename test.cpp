...
void fill_address(arp_packet *buf, unsigned char src_mac[], unsigned char src_ip[], char dest_mac[], unsigned char dest_ip[])
{
	int i;
	for(i = 0; i < 6; i++)
	{
...
void fill_ethernet(arp_packet *buf, char dest[], unsigned char src[])
{
	int i;
	for(i = 0; i < 6; i++)
	{
...
int main(int argc, char *argv[]) {
	...
	fill_ethernet(buf, BROADCAST, my_mac);
	packet_to_arp_request(buf);
	fill_address(buf, my_mac, src_ip, UNKNOW, dest_ip); 
